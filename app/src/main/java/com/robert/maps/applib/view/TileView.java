...
//				mPrevScaleFactor = 1.0;
			...
			//modify by self 20161005
			//disable zoom by scale
			//setZoomLevel(zoom);
		}
...
/*			final BoundingBoxE6 bb = Util.getBoundingBoxFromMapTile(underGeopointTileCoords,
			...
			//由於 HK1980Grid的Y軸由大到小，而Google地圖由小到大，所以計算tileDiffY要方向相反
			int tileDiffX,tileDiffY;

			tileDiffX = centerMapTileCoords[LONGITUDE]
					- underGeopointTileCoords[LONGITUDE];

			if (mTileSource.PROJECTION==4){
				tileDiffY = underGeopointTileCoords[LATITUDE]
						- centerMapTileCoords[LATITUDE];
			}else{
				tileDiffY = centerMapTileCoords[LATITUDE]
						- underGeopointTileCoords[LATITUDE];
			}
			//**************
			final int underGeopointTileScreenLeft = upperLeftCornerOfCenterMapTile.x
			...
			//modify by self 20160916
			//使用HK1980Grid or CentralMapGrid or Taiwan 計算偏移量
			int x,y;
			switch (mTileSource.PROJECTION){
				case 4:
					x = underGeopointTileScreenLeft
							+ HK1980GRID.relativePositionInCenterMapTile[LONGITUDE];
					y = underGeopointTileScreenTop
							+ HK1980GRID.relativePositionInCenterMapTile[LATITUDE];
					break;
				case 5:
					x = underGeopointTileScreenLeft
							+ CentralMapGrid.relativePositionInCenterMapTile[LONGITUDE];
					y = underGeopointTileScreenTop
							+ CentralMapGrid.relativePositionInCenterMapTile[LATITUDE];
					break;
				case 6:
					x = underGeopointTileScreenLeft
							+ TaiwanGrid.relativePositionInCenterMapTile[LONGITUDE];
					y = underGeopointTileScreenTop
							+ TaiwanGrid.relativePositionInCenterMapTile[LATITUDE];
					break;
				case 7:
					x = underGeopointTileScreenLeft
							+ HKMAP2GRID.relativePositionInCenterMapTile[LONGITUDE];
					y = underGeopointTileScreenTop
							+ HKMAP2GRID.relativePositionInCenterMapTile[LATITUDE];
					break;
				default:
					//modify by self 20160916
					//move code from top to bottom
					final BoundingBoxE6 bb = Util.getBoundingBoxFromMapTile(underGeopointTileCoords,
							zoomLevel, mTileSource.PROJECTION);
