...
public class GeoDatabase implements PoiConstants{
	...
	protected SQLiteDatabase getDatabase() {
		...
		//modify by self: change getWritableDatabase() to ...
		SQLiteDatabase db;
		try {
...
//					+ newVersion);
	...
	public Cursor getHikingTrailListCursor(final String units) {
		return getHikingTrailListCursor(units, "trackid ASC");
	}

	public Cursor getHikingTrailListCursor(final String units, final String sortColNames) {
		if (isDatabaseReady()) {
			//
			return mDatabase.rawQuery(String.format(STAT_getHikingTrailList+sortColNames, units), null);
		}
	...
	public void updateHikingTrail(final int id, final String name, final String descr, final int show, final int cnt, final double distance, final double duration, final int category, final int activity, final Date date, final String style) {
		if (isDatabaseReady()) {
			final ContentValues cv = new ContentValues();
			cv.put(NAME, name);
			cv.put(DESCR, descr);
			...
			cv.put(STYLE, style);
			final String[] args = {Integer.toString(id)};
			this.mDatabase.update(HIKING_TRAILS, cv, UPDATE_TRACKS, args);
		}
	}
	...
	//add by self 20160921
	public Cursor getHikingTrailChecked() {
		if (isDatabaseReady()) {
			//
			return mDatabase.rawQuery(STAT_getHikingTrailChecked, null);
		}
	...
	//add by self 20160921
	public Cursor getHikingTrail(final long id) {
		if (isDatabaseReady()) {
			final String[] args = {Long.toString(id)};
			//
			return mDatabase.rawQuery(STAT_getHikingTrail, args);
		}
	...
	//add by self 20160921
	public Cursor getHikingTrailPoints(final long id) {
		if (isDatabaseReady()) {
			final String[] args = {Long.toString(id)};
			//
			return mDatabase.rawQuery(STAT_getHikingTrailPoints, args);
		}
	...
	public void setHikingTrailChecked(final int id){
		if (isDatabaseReady()) {
			final String[] args = {Long.toString(id)};
			mDatabase.execSQL(STAT_setHikingTrailChecked_1, args);
		}
	}

	public void setHikingTrailUnCheckedAll(){
		if (isDatabaseReady()) {
			mDatabase.execSQL(STAT_setHikingTrailChecked_2);
		}
	}
	//**************
	...
	//modify by self 20160924
	public void deleteHikingTrail(final int id) {
		if (isDatabaseReady()) {
			beginTransaction();
			final String[] args = {Long.toString(id)};
			mDatabase.execSQL(STAT_deleteHikingTrail_1, args);
			mDatabase.execSQL(STAT_deleteHikingTrail_2, args);
			commitTransaction();
		}
	}
