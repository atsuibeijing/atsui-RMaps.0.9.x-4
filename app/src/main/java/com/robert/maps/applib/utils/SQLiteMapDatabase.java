...
public class SQLiteMapDatabase implements ICacheProvider {
	...
	//Increase SQL DB size
	private static final long MAX_DATABASE_SIZE = 8000 * 1024 * 1024; // 8.0GB
	//****************
	...
	//modify by self 20161004
	private Context ctx;
	public SQLiteMapDatabase(Context ctx){this.ctx=ctx;}
	public SQLiteMapDatabase(){	}
	//*****************
	...
	private void initDatabaseFiles(final String aFileName, final boolean aCreateNewDatabaseFile) throws RException {
		...
		//modify by self 20161101
		if(aFileName.contains("MyMaps")){
			String SQLFileName="";
			...
			mBaseFile = new File(SQLFileName);
		}else{
		...
		if(folder != null) {
			...
			if(files != null) {
				...
				for (int i = 0; i < files.length; i++) {
					if(files[i].getName().startsWith(mBaseFile.getName()) && !files[i].getName().endsWith(JOURNAL)) {
						try {
							//modify by self 20161015: for exfat can not be mounted as R/W
							//mDatabase[j] = new CashDatabaseHelper(null, files[i].getAbsolutePath()).getWritableDatabase();
							mDatabase[j] = new CashDatabaseHelper(null, files[i].getAbsolutePath()).getReadableDatabase();
