...
*/
						...
						//modify by self 20160916
						//由於 HK1980Grid的Y軸由大到小，而Google地圖由小到大，所以乘以-1,將Y方向調轉
						if(mTileSource.PROJECTION==4) {
							mapTileCoords[LATITUDE] = centerMapTileCoords[LATITUDE] + y*-1;
						}else{
							mapTileCoords[LATITUDE] =centerMapTileCoords[LATITUDE] + y;
						}
						mapTileCoords[LONGITUDE] = centerMapTileCoords[LONGITUDE] + x;
						//*****
	...
	public Point getUpperLeftCornerOfCenterMapTileInScreen(TileView tileView, final int[] centerMapTileCoords, final int tileSizePx, final double offsetLat, final double offsetLon, final Point reuse) {
		...
		//modify by self 20160916
		//Remove the following original code
/*		final BoundingBoxE6 bb = Util.getBoundingBoxFromMapTile(centerMapTileCoords,
		...
		//modify by self 20160916
		//按照HK1980Grid來計算目標在屏幕中心點的坐標
		int centerMapTileScreenLeft;
		int centerMapTileScreenTop;

		switch (mTileSource.PROJECTION){
			case 4:
				centerMapTileScreenLeft = viewWidth_2 - HK1980GRID.relativePositionInCenterMapTile[LONGITUDE] ;
				centerMapTileScreenTop = viewHeight_2 - HK1980GRID.relativePositionInCenterMapTile[LATITUDE];
				break;
			case 5:
				centerMapTileScreenLeft = viewWidth_2 - CentralMapGrid.relativePositionInCenterMapTile[LONGITUDE] ;
				centerMapTileScreenTop = viewHeight_2 - CentralMapGrid.relativePositionInCenterMapTile[LATITUDE];
				break;
			case 6:
				centerMapTileScreenLeft = viewWidth_2 - TaiwanGrid.relativePositionInCenterMapTile[LONGITUDE] ;
				centerMapTileScreenTop = viewHeight_2 - TaiwanGrid.relativePositionInCenterMapTile[LATITUDE];
				break;
			case 7:
				centerMapTileScreenLeft = viewWidth_2 - HKMAP2GRID.relativePositionInCenterMapTile[LONGITUDE] ;
				centerMapTileScreenTop = viewHeight_2 - HKMAP2GRID.relativePositionInCenterMapTile[LATITUDE];
				break;
			default:
				final BoundingBoxE6 bb = Util.getBoundingBoxFromMapTile(centerMapTileCoords,
						tileView.getZoomLevel(), mTileSource.PROJECTION);
				final float[] relativePositionInCenterMapTile = bb
						.getRelativePositionOfGeoPointInBoundingBoxWithLinearInterpolation(
								tileView.mLatitudeE6 + (int)(1E6 * offsetLat), tileView.mLongitudeE6 + (int)(1E6 * offsetLon), null);
				centerMapTileScreenLeft = viewWidth_2
						- (int) (0.5f + (relativePositionInCenterMapTile[LONGITUDE] * tileSizePx));
				centerMapTileScreenTop = viewHeight_2
						- (int) (0.5f + (relativePositionInCenterMapTile[LATITUDE] * tileSizePx));
		}
		//*****
