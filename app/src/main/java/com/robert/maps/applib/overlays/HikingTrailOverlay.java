...
public class HikingTrailOverlay extends TileViewOverlay {
    ...
    private HikingTrailThread mThread;
    ...
    protected ExecutorService mThreadExecutor = Executors.newSingleThreadExecutor(new SimpleThreadFactory("HikingTrailOverlay"));

    private class HikingTrailThread extends Thread {

        @Override
        public void run() {
            Ut.d("run HikingTrailThread");

            mPaths = null;

            if(mTracks == null){
                mTracks = mPoiManager.getHikingTrailChecked(false);
                if(mTracks == null){
                    Ut.d("HikingTrail is null. Stoped??");
                    mThreadRunned = false;
                    mStopDraw = true;
                    return;
                }
                Ut.d("HikingTrail loaded");
            }
            ...
            try {
                ...
                for(int i = 0; i < mTracks.length; i++) {
                    if (mTracks[i] != null) {
                        try {
                            mPaths[i] = mProjection.toPixelsTrackPoints(mPoiManager.getGeoDatabase().getHikingTrailPoints(mTracks[i].getId()), mBaseCoords, mBaseLocation);
                            mPaints[i] = new Paint();
    ...
    public HikingTrailOverlay(MainActivity mainActivity, PoiManager poiManager, Handler aHandler) {
        ...
        mLastZoom = -1;
        mThread = new HikingTrailThread();
        mThread.setName("HikingTrail thread");
    }
