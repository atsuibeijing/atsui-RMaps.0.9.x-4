...
public class TrackActivity extends Activity implements OnTrackStyleChangedListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(id < 0){
			...
			//modify by self 20160924
			if(activity.equals("HikingTrails")){
				mTrack = mPoiManager.getHikingTrail(id);
			}else{
		...
		//modify by self 20160924
		((Button) findViewById(R.id.defaultButton))
				.setOnClickListener(new OnClickListener() {
					public void onClick(View v) {
						doDefaultAction();
					}
	...
	private void doSaveAction() {
		...
		//modify by self 20160924
		if(activity.equals("HikingTrails")){
			mPoiManager.updateHikingTrail(mTrack);
		}else{
	...
	private void doDefaultAction() {
		...
		//modify by self 20160924
		if(activity.equals("HikingTrails")){
			mPoiManager.updateHikingTrail(mTrack,"default");
		}else{
			mPoiManager.updateTrack(mTrack,"default");
		}
		finish();
		Toast.makeText(TrackActivity.this, R.string.message_saved, Toast.LENGTH_SHORT).show();
	}
