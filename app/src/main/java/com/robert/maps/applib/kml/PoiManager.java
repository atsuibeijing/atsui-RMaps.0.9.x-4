...
public class PoiManager implements PoiConstants {
	...
	public void updateTrack(Track track, String flag) {
	...
	//modify by self 20160924
	public void updateHikingTrail(Track track) {
		updateHikingTrail(track,"");
	}
	public void updateHikingTrail(Track track, String flag) {
		if(track.getId() < 0){
			Toast.makeText(mCtx.getApplicationContext(), "No Hiking Trail ID %d was found !"+track.getId(), Toast.LENGTH_LONG).show();

		} else
			if(flag.equals("default")){
				mGeoDatabase.updateHikingTrail(track.getId(), track.Name, track.Descr, track.Show ? ONE : ZERO, track.Cnt, track.Distance, track.Duration, track.Category, track.Activity, track.Date, "");
			}else{
				mGeoDatabase.updateHikingTrail(track.getId(), track.Name, track.Descr, track.Show ? ONE : ZERO, track.Cnt, track.Distance, track.Duration, track.Category, track.Activity, track.Date, track.Style);
			}
	...
	//addb by self 20160921
	public Track[] getHikingTrailChecked(){
		return getHikingTrailChecked(true);
	}

	public Track[] getHikingTrailChecked(final boolean aNeedPoints){
		mStopProcessing = false;
		Track tracks[] = null;
		Cursor c = mGeoDatabase.getHikingTrailChecked();
		if (c != null) {
			tracks = new Track[c.getCount()];
			final String defStyle = PreferenceManager.getDefaultSharedPreferences(mCtx).getString("pref_track_style", "");
			...
			if (c.moveToFirst())
				do {
					...
					if (aNeedPoints) {
						Cursor cpoints = mGeoDatabase.getHikingTrailPoints(tracks[pos].getId());
						if (cpoints != null) {
							if (cpoints.moveToFirst()) {
								do {
									if (Stop()) {
	...
	public Track getHikingTrail(int id){
		Track track = null;
		Cursor c = mGeoDatabase.getHikingTrail(id);
		if (c != null) {
			if (c.moveToFirst()) {
				final String defStyle = PreferenceManager.getDefaultSharedPreferences(mCtx).getString("pref_track_style", "");
				String style = c.getString(9);
			...
			c = mGeoDatabase.getHikingTrailPoints(id);
			if (c != null) {
				if (c.moveToFirst()) {
					do {
						track.AddTrackPoint();
...
//*****************************
	...
	public void setHikingTrailChecked(int id) {
		mGeoDatabase.setHikingTrailChecked(id);
	}

	public void setHikingTrailUnCheckedAll() {
		mGeoDatabase.setHikingTrailUnCheckedAll();
	}
	//**********
	...
	//modify by self 20160924
	public void deleteHikingTrail(int id) {
		mGeoDatabase.deleteHikingTrail(id);
